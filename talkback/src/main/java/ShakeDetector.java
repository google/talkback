...
public class ShakeDetector implements SensorEventListener {
  ...
  public ShakeDetector(FullScreenReadController fullScreenReadController, TalkBackService context) {
  ...
  /**
   * Starts polling the accelerometer for shake detection. If the feature has not be enabled by
   * calling {@link #setEnabled(boolean)}, calling this method is a no-op.
   */
  ...
  public void onSensorChanged(SensorEvent event) {
    ...
    if (deltaT > MOVEMENT_WINDOW) {
      ...
      if ((threshold > 0) && (speed >= threshold)) {
        EventId eventId = EVENT_ID_UNTRACKED; // Performance not tracked for shake events.
