...
public class NodeSearch {
  /**
   * A formatter that determines the text to display given the search query, and the size at which
   * that text should be displayed.
   */
  ...
  public NodeSearch(
      ...
      SearchTextFormatter textFormatter) {
   ...
   */
  public boolean tryAddQueryText(CharSequence newText, EventId eventId) {
   ...
   *
   * @return {@code true} if a character was successfully deleted, or {@code false} if there was no
   *     character to delete.
   */
  ...
  public String getMatchText() {
    ...
    final CharSequence nodeText = CustomLabelManager.getNodeText(currentMatch, mLabelManager);
    if (nodeText == null) {
      return "";
    }
   ...
   */
  public boolean nextResult(int direction, EventId eventId) {
    ...
    try {
      ...
      return PerformActionUtils.performAction(
          focusableNext, AccessibilityNodeInfo.ACTION_ACCESSIBILITY_FOCUS, eventId);
    } finally {
      if (focusableNext != null) {
        focusableNext.recycle();
      }
   ...
   * @return {@code true} if a match was found, or {@code false} otherwise.
   */
  private boolean evaluateSearch(EventId eventId) {
    // First check if current selected result still matches.
    return nodeMatchesQuery(mMatchedNode.get())
        || nextResult(NodeFocusFinder.SEARCH_FORWARD, eventId);
  }
   ...
   * @return {@code true} if the node's description contains the current query text (ignoring case),
  ...
  private boolean nodeMatchesQuery(AccessibilityNodeInfoCompat node) {
    ...
    CharSequence nodeText = CustomLabelManager.getNodeText(node, mLabelManager);
    if (nodeText == null) {
      return false;
    }
  ...
  private static class SearchOverlay extends SimpleOverlay implements DialogInterface {
    ...
    public SearchOverlay(
        Context context, StringBuilder queryText, SearchTextFormatter textFormatter) {
      super(context);
  ...
  private static class SearchView extends SurfaceView {
    ...
    /**
     * The search query text. Synced to the StringBuilder in NodeSearch so we shouldn't modify it
     * here.
     */
    ...
    public SearchView(Context context, StringBuilder queryText, SearchTextFormatter textFormatter) {
      super(context);
    ...
    public void invalidate() {
      ...
      canvas.drawText(
          mTextFormatter.getDisplayText(mQueryText.toString()), width / 2.0f, height / 2.0f, paint);
