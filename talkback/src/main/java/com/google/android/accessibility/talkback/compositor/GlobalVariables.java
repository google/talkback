...
public class GlobalVariables extends TimedFlags implements ParseTree.VariableDelegate {
  ...
  public boolean getBoolean(int variableId) {
    switch (variableId) {
      ...
      case COLLECTION_EXISTS:
        return collectionState.doesCollectionExist();
      ...
      case COLLECTION_PAGER_ITEM_IS_HEADING:
        {
          CollectionState.PagerItemState itemState = collectionState.getPagerItemState();
          return (itemState != null) && itemState.isHeading();
        }
  ...
  public @Nullable CharSequence getString(int variableId) {
    switch (variableId) {
      ...
      case COLLECTION_NAME:
        return collectionState.getCollectionName();
  ...
  private CharSequence getCollectionListItemCount() {
    if (hasBothCount()) {
      if (isVerticalAligned() && collectionState.getCollectionRowCount() >= 0) {
        return quantityCharSequence(
            R.plurals.template_list_total_count,
            collectionState.getCollectionRowCount(),
            collectionState.getCollectionRowCount());
      } else if (isHorizontalAligned() && collectionState.getCollectionColumnCount() >= 0) {
        return quantityCharSequence(
            R.plurals.template_list_total_count,
  ...
  private CharSequence getCollectionName(int stringResId, int withNameStringResId) {
    if (collectionState.getCollectionName() == null) {
      ...
      return mContext.getString(withNameStringResId, collectionState.getCollectionName());
    }
  }
