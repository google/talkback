...
enum class A11yAction(val gestureMappingReference: Int) {
    ...
    companion object {
        const val granularityParameter = "mode"
        fun fromString(string: String): A11yAction =
                values().firstOrNull { it.name.lowercase() == string.lowercase() } ?: UNASSIGNED

        fun granularityFrom(string: String): SelectorController.Granularity =
                SelectorController.Granularity.values().firstOrNull {
                    it.name.lowercase() == string
                } ?: SelectorController.Granularity.DEFAULT
    }
}