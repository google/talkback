 ...
 * <p>
 */
public class IntegratedTapDetector
    implements SensorEventListener, ThreeDSensorTapDetector.TapListener {
  ...
  public IntegratedTapDetector(
    ...
    if (accelTapDetector == null) {
      ...
      accelTapDetector =
          new ThreeDSensorTapDetector(this, maxRange, ThreeDSensorTapDetectorType.ACCELEROMETER);
    }
    ...
    if (gyroTapDetector == null) {
      ...
      gyroTapDetector =
          new ThreeDSensorTapDetector(this, maxRange, ThreeDSensorTapDetectorType.GYROSCOPE);
    }
  ...
  private void processIntegratedQueueAsSingleAndDoubleTaps(long timestamp) {
    while (integratedTapEventQueue.size() >= 2) {
      ...
      if (!tapAllowedAt(olderTap.nanos)) {
        if (DEBUG) {
          Log.v(
              "Picidae",
              String.format(
                  "Disallowing tap at time %d with quality %f", olderTap.nanos, olderTap.quality));
        }
        continue;
      }
      ...
      if (olderTap.quality >= mMinTapQuality) {
        ...
        Log.v(
            "Picidae",
            String.format(
                "Discarding tap at time %d with quality %f", olderTap.nanos, olderTap.quality));
      }
    }
    ...
    if (((tap = integratedTapEventQueue.peek()) != null)
      ...
      if ((tap.quality >= mMinTapQuality) && tapAllowedAt(tap.nanos)) {
        ...
        Log.v(
            "Picidae",
            String.format("Discarding tap at time %d with quality %f", tap.nanos, tap.quality));
      }
    }
  }
