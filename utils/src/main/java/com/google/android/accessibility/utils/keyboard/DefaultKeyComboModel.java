...
public class DefaultKeyComboModel implements KeyComboModel {
  ...
  private void addKeyCombos() {
    ...
    addKeyCombo(mContext.getString(R.string.keycombo_shortcut_other_talkback_context_menu));
  ...
  public long getDefaultKeyComboCode(String key) {
    ...
    if (!FeatureSupport.hasAccessibilityShortcut(mContext)
        && key.equals(mContext.getString(R.string.keycombo_shortcut_global_suspend))) {
    ...
    if (key.equals(mContext.getString(R.string.keycombo_shortcut_other_talkback_context_menu))) {
      if (mIsArc) {
        return KeyComboManager.getKeyComboCode(NO_MODIFIER, KeyEvent.KEYCODE_ENTER);
      } else {
        return KeyComboManager.getKeyComboCode(NO_MODIFIER, KeyEvent.KEYCODE_SPACE);
