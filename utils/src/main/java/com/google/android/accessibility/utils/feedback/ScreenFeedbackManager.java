...
import com.google.android.accessibility.utils.WindowEventInterpreter.Announcement;
...
public class ScreenFeedbackManager
  ...
  public CharSequence getWindowTitle(int windowId) {
    return interpreter.getWindowTitleForFeedback(windowId);
  }
      ...
      if (announcement != null) {
        ...
        feedback.addPart(
            new FeedbackPart(announcement.text())
                .earcon(true)
                .forceFeedbackAudioPlaybackActive(!announcement.isFromVolumeControlPanel())
                .forceFeedbackMicrophoneActive(!announcement.isFromVolumeControlPanel())
                .forceFeedbackSsbActive(announcement.isFromInputMethodEditor()));
      }
