...
public class AutomationUtils {
  ...
  /**
   * Attempts to perform an accessibility action on a view that is an instance of the class {@code
   * className}, the text or content description {@code text}, and exists under a parent view {@code
   * root}.
   *
   ...
   */
  public static boolean performActionOnView(
    ...
    return PerformActionUtils.performAction(node, action, arguments, eventId);
  }

  /**
   * Returns a list of nodes under the {@code root} node that match the class specified by {@code
   * className} and exactly match the text or content description specified by {@code text}.
   */
  ...
  /**
   * Returns whether a node matches the class specified by {@code className} and exactly match the
   * text or content description specified by {@code text}.
   */
  private static boolean nodeMatchesFilter(
      AccessibilityNodeInfoCompat node, CharSequence referenceClassName, String findText) {
