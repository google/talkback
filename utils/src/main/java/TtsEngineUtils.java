...
/**
 * Support class for querying the list of available engines on the device and deciding which one to
 * use etc.
 *
 * <p>Comments in this class the use the shorthand "system engines" for engines that are a part of
 * the system image.
 *
 * <p>Based on hidden framework class {@code android.speech.tts.TtsEngines}.
 */
public class TtsEngineUtils {
  ...
  /**
   * Returns the package name of the default TTS engine. If the user has set a default, and the
   * engine is available on the device, the default is returned. Otherwise, the highest ranked
   * engine is returned. If no system engine is present, returns {@code null}.
   *
   * @return the package name of the default TTS engine, or {@code null} if no default or system
   *     engine is present.
   */
  public static String getDefaultEngine(Context context) {
    final ContentResolver resolver = context.getContentResolver();
    final String defaultEngine =
        Settings.Secure.getString(resolver, Settings.Secure.TTS_DEFAULT_SYNTH);
  ...
  /**
   * Returns the default locale for a given TTS engine. Attempts to read the value from {@link
   * SettingsCompatUtils.SecureCompatUtils#TTS_DEFAULT_LOCALE}, failing which the old style value
   * from {@link Settings.Secure#TTS_DEFAULT_LANG} is read. If both these values are empty, the
   * default phone locale is returned.
   *
   * @param engineName the engine to return the locale for.
   * @return the locale string preference for this engine. Will be non null and non empty.
   */
  @SuppressWarnings("javadoc")
  public static Locale getDefaultLocaleForEngine(Context context, String engineName) {
    final ContentResolver resolver = context.getContentResolver();
    final String defaultLocalePref =
  ...
  /**
   * Gets a list of all installed TTS engines sorted by priority (see {@link
   * #ENGINE_PRIORITY_COMPARATOR}).
   *
   * @return A sorted list of engine info objects. The list can be empty, but never {@code null}.
   */
  ...
  /**
   * Parses the contents of {@link Engine#EXTRA_AVAILABLE_VOICES} and returns a unmodifiable list of
   * {@link Locale}s sorted by display name. See {@link #LOCALE_COMPARATOR} for sorting information.
   *
   * @param availableLanguages A list of locale strings in the form {@code
  ...
  /**
   * Returns the engine info for a given engine name. Note that engines are identified by their
   * package name.
   */
  ...
  /**
   * @return information for the highest ranked system engine or {@code null} if no TTS engines were
   ...
   *     {@link Settings.Secure#TTS_DEFAULT_COUNTRY} and {@link
   *     Settings.Secure#TTS_DEFAULT_VARIANT}. If no such locale is set, then return the default
   *     phone locale.
   */
  ...
  /** Compares locales in case-insensitive ascending order based on their display name. */
  ...
  /**
   * Engines that are a part of the system image are always lesser than those that are not. Within
   * system engines / non system engines the engines are sorted in order of their declared priority.
   */
  private static final Comparator<TtsEngineInfo> ENGINE_PRIORITY_COMPARATOR =
      new Comparator<TtsEngineInfo>() {
        @Override
  ...
  /**
   * Information about an installed text-to-speech engine. Cloned from {@link
  ...
  public static class TtsEngineInfo {
    ...
    /**
     * The priority the engine declares for the intent filter {@code
