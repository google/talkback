  ...
  public Translator create(Context context, Code code, boolean isContractedOn) {
    return new Translator() {
      ...
      public TranslationResult translateToBraille(String text, int cursorPosition) {
        BrailleWord makeupBrailleWord =
            new BrailleWord(Collections.nCopies(text.length(), BRAILLE_CHARACTER_DOT_1));
        int[] makeupArray = IntStream.range(0, text.length()).toArray();
        return new TranslationResult(
            makeupBrailleWord.toByteArray(), makeupArray, makeupArray, cursorPosition);
      }
    };
  }
}
