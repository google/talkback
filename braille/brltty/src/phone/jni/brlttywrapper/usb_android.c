...
#include "third_party/brltty/Headers/bitfield.h"
#include "third_party/brltty/Headers/queue.h"

#include "third_party/brltty/Headers/io_usb.h"
#include "third_party/brltty/Programs/usb_internal.h"
...
#define JAVA_SIG_CONTEXT JAVA_SIG_OBJECT("android/content/Context")
...
static jobject* globalContext = NULL;

void
usbAndroidSetContext(jobject* context) {
  globalContext = context;
}

static int
usbFindHelperClass (JNIEnv *env) {
  return findJavaClass(env, &usbHelperClass, "com/google/android/accessibility/braille/brltty/UsbHelper");
}
...
usbGetDeviceIterator (JNIEnv *env) {
  if (usbFindHelperClass(env)) {
    ...
    if (findJavaStaticMethod(env, &method, usbHelperClass, "getDeviceIterator",
                             JAVA_SIG_METHOD(JAVA_SIG_ITERATOR, JAVA_SIG_CONTEXT))) {
      jobject iterator = (*env)->CallStaticObjectMethod(env, usbHelperClass, method, globalContext);
...
usbOpenDeviceConnection (JNIEnv *env, jobject device) {
  ...
  if (usbFindHelperClass(env)) {
    ...
    if (findJavaStaticMethod(env, &method, usbHelperClass, "openDeviceConnection",
                             JAVA_SIG_METHOD(JAVA_SIG_USB_CONNECTION, JAVA_SIG_CONTEXT JAVA_SIG_USB_DEVICE))) {
      jobject connection = (*env)->CallStaticObjectMethod(env, usbHelperClass, method, globalContext, device);
