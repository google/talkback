...
public class BtConnector {
  ...
  public interface Callback {
     ...
     */
    void onConnectFailure(BluetoothDevice device, Exception exception);
  }
  ...
  public BtConnector(BluetoothDevice device, Callback callback) {
  ...
  /**
   * Connect to the device. If a failure occurs, {@link Callback#onConnectFailure(BluetoothDevice,
  ...
  public void connect() {
    try {
      ...
      connectingSocket = BluetoothDevices.createInsecureRfcommSocketToServiceRecord(device);
    ...
    public void run() {
      try {
        ...
        BtConnection btConnection =
            new BtConnection(device, connectingSocket, inputStream, outputStream);

        // Nullifying the local copy of the socket is critical because we are transferring
        // ownership of the socket to the BtConnection object. So any subsequent invocation
        // of shutdown will correctly have no effect on the socket because we no longer own
        // it.
  ...
  public void shutdown() {
    callback = null;
    try {
      // Purposely allowing for NPE which will be caught; avoids need for synchronized.
