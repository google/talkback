...
public class DefaultBrailleRule implements BrailleRule {
  ...
  public void format(Editable result, Context context, AccessibilityNodeInfoCompat node) {
    ...
    StringUtils.appendWithSpaces(result, filterNonPrintCharacter(text));
  ...
  private static CharSequence filterNonPrintCharacter(CharSequence text) {
    SpannableStringBuilder stringBuilder = new SpannableStringBuilder(text);
    for (int i = text.length() - 1; i >= 0; i--) {
      // Filter characters like \u202A and \u202C which are non-printing characters.
