  ...
  static class Builder {
    ...
    private TranslationResult translateIfPossible(CharSequence text, int cursorPosition) {
      TranslationResult result =
          translator.translate(text, cursorPosition, /* computerBrailleAtCursor= */ false);
      if (TranslateHelper.isNewLineBlank(translator)) {
        result = TranslateHelper.replaceNewLine(result);
      }
      if (result == null) {
    ...
    public AssembledResult build() {
      ...
      int start =
          textToBraillePositions.get(resultForDisplay.textSelection().start + textDisplacement);
      int end = textToBraillePositions.get(resultForDisplay.textSelection().end + textDisplacement);
    ...
    private void appendTextFieldTextBeforeSelection() {
      ...
      boolean cursorAtEnd =
          textSelectionRange.getLower() == NO_CURSOR
              || textSelectionRange.getLower() == textFieldText.length();
    ...
    private void appendTextFieldTextBetweenSelection() {
      if (TextUtils.isEmpty(textFieldText)
          || textSelectionRange.getLower().intValue() == textSelectionRange.getUpper().intValue()) {
        return;
      }
      int textIndexBeforeCursor =
    ...
    private void extendTextFieldTextForCursor() {
      boolean extendForCursor =
          textSelectionRange.getLower().intValue() == textSelectionRange.getUpper().intValue()
              && textSelectionRange.getUpper() == textFieldText.length()
              && holdingsWord.isEmpty();
      if (extendForCursor) {
        appendSpace();
      }
    }
